...
<body class="bg-main min-h-screen relative">
        ...
        <div class="px-4 md:px-6 py-4 flex justify-between items-center max-w-7xl mx-auto">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Pending orders waiting for completion</p>
                </div>
            </div>
            <a href="pos.php" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-blue-600/20 transition-all flex items-center gap-2 block max-w-max">
                New Sale
    ...
    <main class="pt-32 pb-20 px-4 md:px-6 max-w-7xl mx-auto relative z-10 animate-fade">
        <?php if (empty($drafts)): ?>
            <div class="glass-card p-16 text-center shadow-lg border-2 border-white max-w-2xl mx-auto">
                <div class="w-20 h-20 bg-slate-100/50 rounded-full flex items-center justify-center mx-auto mb-6">
                ...
                <h3 class="text-xl font-black text-slate-700 uppercase tracking-tight mb-2">No Pending Sales Found</h3>
                <p class="text-sm font-bold text-slate-400">You don't have any pending sales at the moment.</p>
                <a href="pos.php" class="inline-flex mt-8 bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-blue-600/20 transition-all items-center gap-2">
                    Go To POS
                </a>
            </div>
        <?php else: ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($drafts as $d): ?>
                    <a href="pos.php?draft_id=<?php echo $d['id']; ?>" class="glass-card p-6 border-2 border-white shadow-[0_8px_30px_-12px_rgba(0,0,0,0.1)] hover:shadow-[0_15px_40px_-10px_rgba(37,99,235,0.15)] transition-all duration-300 hover:-translate-y-1.5 group block relative overflow-hidden">
                        <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-500/5 to-indigo-500/5 rounded-bl-full -z-10 group-hover:from-blue-500/10 group-hover:scale-110 transition-all duration-500"></div>
                        
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <span class="text-[9px] font-black uppercase tracking-[0.2em] text-blue-600 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-md mb-3 inline-block shadow-sm">ID-<?php echo $d['id']; ?></span>
                                <h4 class="text-lg font-black text-slate-800 uppercase tracking-tighter leading-tight group-hover:text-blue-600 transition-colors"><?php echo htmlspecialchars($d['cust_name'] ?: 'Walk-in Customer'); ?></h4>
                                <?php if ($d['cust_name']): ?>
                                    <p class="text-[10px] text-slate-500 font-bold mt-1 tracking-wider"><?php echo htmlspecialchars($d['cust_contact']); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-end border-t border-slate-100 pt-5">
                            <div>
                                <span class="text-[8px] font-black uppercase text-slate-400 tracking-widest block mb-1">Estimated Total</span>
                                <p class="text-2xl font-black text-slate-900 leading-none tracking-tight group-hover:text-blue-600 transition-colors">Rs. <?php echo numberFormat($d['final_amount']); ?></p>
                            </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-50 flex justify-between items-center text-[10px] font-bold text-slate-400">
                            <span>Saved: <?php echo date('M d, Y', strtotime($d['created_at'])); ?></span>
